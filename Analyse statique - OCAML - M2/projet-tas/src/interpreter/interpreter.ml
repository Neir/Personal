...
(struct
  ...
  let rec eval_stat (a:t) ((s,ext):stat ext) : t = 
    ...
    | AST_while (e,s) ->
	let d = ref (!widen_delay) in
	let u = ref (!loop_unrolling) in
	
	let unroll x =
	let a = ref x in
	  for i=1 to !u do
	    a := eval_stat (filter !a e true) s;
	  done;
	  !a
	in
        (* simple fixpoint *)
         ...
         *)  
	let unrolled = unroll a in
	  let f x =
	    let jn = eval_stat (filter x e true) s in
	    if (!d > 0) then
	    begin
	      d := !d - 1;
	      D.join x jn
	...
	let t = filter a e true in
	  if not (D.subset a t) || not (D.subset t a) then
	     Format.printf "Assertion FAILED with environment: %a@\n" D.print_all a;
          t
